...
class Ajax extends CI_Controller {
	...
	public function download(){
		if(isset($_POST['id']) && isset($_POST['pass'])){
			$authorized = $this->Files_model->downloadfile($this->input->post('id'),$this->input->post('pass'));
			if($authorized){
				echo json_encode(array('success' => 'grantred','name'=>$authorized['name'],'src'=>$authorized['src']));
			}
			else {
				header('Content-Type: application/json');
